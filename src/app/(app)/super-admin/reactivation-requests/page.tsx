...
export default function ReactivationRequestsPage() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/reactivation-requests?${params.toString()}`
      );
      if (response.ok) {
  ...
  const getProcessedTodayCount = () => {
    return requests.filter(
      (r) =>
        r.decided_at &&
        new Date(r.decided_at).toDateString() === new Date().toDateString()
    ).length;
  };
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Kelola dan proses permohonan reaktivasi domain dari Admin Daerah
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{getProcessedTodayCount()}</div>
          </CardContent>
        </Card>
      </div>
                    ...
                    <TableRow key={request.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="outline" size="sm" asChild>
                          <Link
                            href={`/super-admin/reactivation-requests/${request.id}?role=Super Admin`}
                          >
