...
function SuperAdminReportsContent() {
    ...
    switch (reportType) {
      ...
      case "applications":
        ...
        reportData.applications.byOpd.forEach((item) => {
          csvContent += `${item.opd},${item.pending},${item.approved},${
            item.rejected
      ...
      default: // overview
        ...
        csvContent += `Permohonan Domain,Approval Rate,${reportData.applications.approvalRate.toFixed(
          2
        )}%\n`;
        ...
        csvContent += `Hosting,Approval Rate,${reportData.hosting.approvalRate.toFixed(
          2
        )}%\n`;
  ...
  const handleExportPDF = () => {
    alert(
      "Export PDF akan segera tersedia. Untuk saat ini, silakan gunakan Export CSV atau Print halaman ini."
    );
  };
