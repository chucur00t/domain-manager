...
import { MOCK_DOMAINS, MOCK_APPLICATIONS } from '@/lib/mock-data';
import {
  ChartConfig,
  ChartContainer,
  ChartTooltip,
...
export function KabidDashboard() {
  const pendingApprovalCount = MOCK_APPLICATIONS.filter(a => a.status === 'pending_approval').length;
  ...
  ].filter(item => item.value > 0), [approvedCount, rejectedCount]);

  const recentPendingApplications = MOCK_APPLICATIONS
