...
export const navItems: NavItem[] = [
  ...
  // ADMIN DAERAH: Menu Pengajuan (untuk mengajukan permohonan)
  {
    ...
    subItems: [
      {
        href: (role) => `/applications?role=${encodeURIComponent(role)}`,
        label: "Ajukan Domain",
        icon: FileText,
        roles: ["Admin Daerah"],
      },
      {
        ...
        label: "Ajukan Hosting",
        icon: Server,
        roles: ["Admin Daerah"],
      },
      {
        href: (role) => `/domains/deactivate?role=${encodeURIComponent(role)}`,
        label: "Deaktivasi Domain",
        icon: Globe,
        roles: ["Admin Daerah"],
      },
      {
  ...
  // Menu Administrasi untuk Super Admin (TANPA Pendaftaran Subdomain/Hosting)
  {
    ...
    subItems: [
      {
        href: (role) => `/super-admin/users?role=${encodeURIComponent(role)}`,
      ...
      {
        href: (role) => `/super-admin/opds?role=${encodeURIComponent(role)}`,
        label: "Manajemen OPD",
        icon: Building,
        roles: ["Super Admin"],
        exact: true,
  ...
  {
    href: (role) => `/domains?role=${encodeURIComponent(role)}`,
    label: "Domain Saya",
    icon: Globe,
    roles: ["Admin Daerah"],
  },
  ...
  {
    href: (role) => `/super-admin/domains?role=${encodeURIComponent(role)}`,
    label: "Manajemen Domain",
    icon: Globe,
    roles: ["Super Admin"],
  },
  ...
  // Super Admin: Laporan
  {
    href: (role) => `/super-admin/reports?role=${encodeURIComponent(role)}`,
    label: "Laporan",
    icon: FileBarChart,
    roles: ["Super Admin"],
  },
  ...
  {
    href: (role) => `/super-admin/audit-trail?role=${encodeURIComponent(role)}`,
    label: "Audit Trail",
    icon: ShieldAlert,
    roles: ["Super Admin"],
  },
  ...
  {
    href: (role) => `/settings?role=${encodeURIComponent(role)}`,
    label: "Pengaturan",
    icon: Settings,
    roles: ["Super Admin", "Admin Daerah"],
  },
...
function NavLinks() {
  ...
  return (
    <nav className="grid items-start gap-1 px-2 text-sm font-medium lg:px-4">
      {getFilteredNavItems(navItems).map(
        ({ href, label, icon: Icon, exact, subItems }) => {
          if (subItems && subItems.length > 0) {
            const filteredSubItems = getFilteredNavItems(subItems);
            if (filteredSubItems.length === 0) return null;

            const isAnySubItemActive = filteredSubItems.some((sub) =>
              isLinkActive(sub.href(role), sub.exact)
            );
