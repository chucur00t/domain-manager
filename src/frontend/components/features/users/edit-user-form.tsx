...
import { useState, type ReactNode, useTransition, useEffect, Suspense, useMemo } from 'react';
...
import { updateUser, deleteUser, updateUserStatus } from '@/backend/actions/users';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
function EditUserFormContent({ user, currentUser, onFormAction, allOpds, isOpen, onOpenChange }: EditUserFormProps) {
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <div className='flex flex-col sm:flex-row gap-4 py-4'>
            <div className='w-full sm:w-1/2 flex flex-col gap-4'>
                ...
                >
                    {isStatusUpdating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : (user.status === 'active' ? <ToggleLeft className="mr-2 h-4 w-4" /> : <ToggleRight className="mr-2 h-4 w-4" />)}
...
export function EditUserForm(props: EditUserFormProps) {
    return (
        <Suspense fallback={<div className='p-2'><Loader2 className="h-4 w-4 animate-spin" /></div>}>
